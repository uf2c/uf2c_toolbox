function [dvarsC1,dvarsC2,dvarsC3,MeanDvars] = bramila_dvars_uf2c(pathFunc,dirname,finalEPI,extGMvar,extWMvar,extCSFvar,DVARthresV)
...
% finalEPIORI = finalEPI;

% finalEPI = finalEPIORI;

% finalEPIresh = reshape(finalEPI,prod([size(finalEPI,1),size(finalEPI,2),size(finalEPI,3)]),size(finalEPI,4));
% finalEPIresh = finalEPIresh';
% for hh = 1:size(finalEPIresh,2)
%     meditmp = mean(finalEPIresh(:,hh));
%     finalEPIresh(:,hh) = detrend(finalEPIresh(:,hh),'linear') + meditmp;
% end
% finalEPIresh = finalEPIresh';
% finalEPI = reshape(finalEPIresh,size(finalEPI,1),size(finalEPI,2),size(finalEPI,3),size(finalEPI,4));
...
if extGMvar
    ...
    save([pathFunc,dirname,filesep,'Motion_Controls_Params',filesep,'DerivativeVariance_GM.mat'],'dvarsC1')
